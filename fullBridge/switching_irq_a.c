	...
	switch(gMachineState)
	{
		...
		case STATE_INIT_RUN:
			...
			if( invt_PWM_Port_Set_flag == 0 ){
				...
				EPwm1Regs.TBPHS.half.TBPHS = (Uint16)(MAX_PWM_CNT * codePwmPhaseInit)  ;
...
//		case STATE_BREAK_OFF:
			...
			if( code_ctrl_mode == 1 ) fullBridgeCtrl();
				...
				EPwm1Regs.TBPHS.half.TBPHS = (Uint16)( MAX_PWM_CNT * reference_out );
			}
			else if( code_ctrl_mode == 2 ){ // mode2LoopCtrl mode
				EPwm1Regs.TBPHS.half.TBPHS =(Uint16)( MAX_PWM_CNT * code_testPwmPhase );
			}
			else{
				EPwm1Regs.CMPA.half.CMPA = MAX_PWM_CNT;
				EPwm2Regs.CMPA.half.CMPA = MAX_PWM_CNT;
