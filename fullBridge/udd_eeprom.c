...
// write data
	...
	I2caRegs.I2CSAR = iSlaveAddr;
  	I2caRegs.I2CDXR = (iMemAddr >> 8) & 0x00ff;			// 상위 바이트 
	...
	StartMs = ulGetNow_mSec();
	while(I2caRegs.I2CSTR.bit.SCD == 0)
	{
		EndMs = ulGetTime_mSec(StartMs);
		if(EndMs > 10)				// 20ms 가 지나면 루프 탈출
		{
			// EEPROM WRITE ERROR
			EndMs = 20;
			break;
...
Uint16 I2CA_ReadData(int iSlaveAddr, int iMemAddr, int * data)
{
   ...
   I2caRegs.I2CMDR.all = 0x6620;			
	...
	StartMs = ulGetNow_mSec();
	while(I2caRegs.I2CSTR.bit.ARDY == 0) 	// jsk 2010.06.21
	{
		EndMs = ulGetTime_mSec(StartMs);
		if(EndMs > 10)				// 20ms 가 지나면 루프 탈출
		{
   ...
   I2caRegs.I2CMDR.all = 0x6C20;			// Send restart as master receiver stop
	...
	EndMs = 0;
	StartMs = ulGetNow_mSec();
	while(I2caRegs.I2CSTR.bit.SCD == 0) 	//jsk
	{
		EndMs = ulGetTime_mSec(StartMs);
		if(EndMs > 10)				// 20ms 가 지나면 루프 탈출
		{
	...
	while( loop_ctrl )	
	{
		...
		if( check == 0){
			...
			if(check){
				...
				snprintf( str,30,"address=%d",address);
				load_sci_tx_mail_box( str );
