// serial_com.c 2009.10.19

#include	<header.h>
#include	<extern.h>
/*
const unsigned char auchCRCHi[ ] = {
 } ;

const unsigned char auchCRCLo[ ] = {
 } ;

*/

/*
unsigned int CRC16( char* puchMsg, unsigned int usDataLen)
{
	unsigned char uchCRCHi = 0xFF ; // high byte of CRC initialized 
	unsigned char uchCRCLo = 0xFF ; // low byte of CRC initialized 
	unsigned uIndex ; 				// will index into CRC lookup table 

	while (usDataLen--) 			// pass through message buffer
	{
		uIndex = uchCRCHi ^ *puchMsg++ ; // calculate the CRC 
		uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
		uchCRCLo = auchCRCLo[uIndex] ;
	}
	// printf("CRC H : %X, CRC L : %X\n", uchCRCHi, uchCRCLo);
	return (uchCRCHi << 8 | uchCRCLo) ;
}
*/

void load_sci_tx_mail_box( char * str)
{
	load_scib_tx_mail_box( str);
	load_scic_tx_mail_box( str);
	delay_msecs(50);
}

void serial_com_proc( int * sci_cmd, double * sci_ref)
{

	if (scib_rx_msg_flag == 1 ){
		scib_cmd_proc( sci_cmd, sci_ref);
		scib_rx_msg_flag = 0;
		return;
	}
	else if (scic_rx_msg_flag == 1 ){
		scic_cmd_proc( sci_cmd, sci_ref);
		scic_rx_msg_flag = 0;
		return;
	}
	else{
		* sci_cmd = CMD_NULL;
		* sci_ref = 0.0;
		return;
	}
}
void sendAllCodeData(void)
{
	UNION32	u32data;
	int check;
	int address,cmd;
	char str[30]={0};

	u32data.dword  = 0.0;

	address = 0;	// start of address

	cmd = CMD_READ_DATA;
	address = 0;
	load_scic_tx_mail_box("\r\n");
	load_scic_tx_mail_box("---DC Power Code List---"); 
	GetTimeAndDateStr(str);
	load_scic_tx_mail_box(str); 
	load_scic_tx_mail_box("\r\n");

	while( address < CODE_GROUP7_END )	
	{
		cmd = CMD_READ_DATA;
		check = get_code_information( address, cmd , & code_inform);

		if( check == 0){
			read_eprom_data( address, & u32data);
//--- disp Disp
			load_scic_tx_mail_box(code_inform.disp);
			load_scic_tx_mail_box_char('\t');
			delay_msecs(10);

			snprintf( str,19,"CODE=%4d",address); 
			load_scic_tx_mail_box(str); 
			load_scic_tx_mail_box_char('\t');
			delay_msecs(10);

			if( code_inform.type == TYPE_DOUBLE )
				snprintf( str,20,"Data =%10.3e",code_inform.code_value.doubles);
			else
				snprintf( str,20,"Data =%10d",code_inform.code_value.ints);

			load_scic_tx_mail_box(str);	
			load_scic_tx_mail_box_char('\t');
			delay_msecs(10);				 

			check = check_code_data( u32data, code_inform);	// check min and max boundary
			if(check){				
				load_sci_tx_mail_box("Code Data Error:");
			}
			load_scic_tx_mail_box("\r\n");
			address++;
		}
		else if( check == -2){
			load_scic_tx_mail_box("\r\n");
			address = ( address / 100 )*100 + 100;

		}
		else address ++;
	}
	load_scic_tx_mail_box("---End of DC-Power Code List---");
}
//======================
// end of serial_com.c
//======================

